...
RUN pip install --no-cache-dir autokeras
...
COPY autokeras_server/pyproject.toml autokeras_server/poetry.lock autokeras_server/
RUN cd yolo_server && poetry install --no-root --no-interaction --no-cache
COPY autokeras_server autokeras_server
...
COPY --from=builder autokeras_server autokeras_server
